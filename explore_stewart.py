...
BEFORE AND AFTER OUTLIERS REMOVED.
    ...
    outlier_cols = ['baths', 'beds', 'sq_ft', 'fullbaths', 'tax_value', 'logerror']
...
def cluster_and_scale(train, validate, test):
    ...
    X_train = train#[['age', 'tax_value', 'sq_ft']]
    X_validate = validate#[['age', 'tax_value', 'sq_ft']]
    X_test = test#[['age', 'tax_value', 'sq_ft']]
    ...
    kmeans.fit(X_train[['beds', 'baths', 'sq_ft']])
    X_train_scaled['cluster'] = kmeans.predict(X_train[['beds', 'baths', 'sq_ft']])
    X_train_scaled['cluster'] = 'cluster_' + X_train_scaled.cluster.astype(str)
    
    X_validate_scaled['cluster'] = kmeans.predict(X_validate[['beds', 'baths', 'sq_ft']])
    X_validate_scaled['cluster'] = 'cluster_' + X_validate_scaled.cluster.astype(str)
    
    X_test_scaled['cluster'] = kmeans.predict(X_test[['beds', 'baths', 'sq_ft']])
    X_test_scaled['cluster'] = 'cluster_' + X_test_scaled.cluster.astype(str)
    
    #visualize clusters and model features with clusters
    plt.figure(figsize = (12, 4))
    sns.scatterplot(data = X_train_scaled, x = 'sq_ft', y = 'baths', size = 'beds', hue = 'cluster', alpha = 0.6)
    plt.title('Clusters Groupings Using `age`, `tax_value`, and `sq_ft`, k=4')
    plt.legend('')
    plt.show();
